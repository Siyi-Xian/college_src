...
class DecisionTreeFactory:
    ...
    def _build_tree_rec(self, node):
        ...
        try:
            ...
            print("Cannot split dataset now")
            assert splits != []
    ...
    def _get_ranked_splits(self, splits):
        ...
        """
        return sorted(splits, key=lambda att: self._calc_information_gain(att[0], att[1]), reverse=True)
    ...
    def _calc_information_gain(self, parent_node, children_nodes):
        ...
        """
        ans = parent_node.get_entropy(self.labels[parent_node.idxs])
        for child_node in children_nodes:
            ans -= len(child_node.idxs) / len(parent_node.idxs) * child_node.get_entropy(self.labels[child_node.idxs])
        print(ans)
        return ans
