    ...
    def isTerminal(self):
        ...
        for i in range(0, len(self.board)):
            for j in range(0, len(self.board[i])):
                if self.board[i][j] == 0:
                    player_one.append((i, j))
                else:
                    player_two.append((i, j))

        for pos in player_one:
            if (pos[0] + 1, pos[1]) in player_one and (pos[0] + 2, pos[1]) in player_one and (
            pos[0] + 3, pos[1]) in player_one:
                return 1
            if (pos[0], pos[1] + 1) in player_one and (pos[0], pos[1] + 2) in player_one and (
            pos[0], pos[1] + 3) in player_one:
                return 1
            if (pos[0] + 1, pos[1] + 1) in player_one and (pos[0] + 2, pos[1] + 2) in player_one and (
            pos[0] + 3, pos[1] + 3) in player_one:
                return 1
            if (pos[0] - 1, pos[1] + 1) in player_one and (pos[0] - 2, pos[1] + 2) in player_one and (
            pos[0] - 3, pos[1] + 3) in player_one:
                return 1

        for pos in player_two:
            if (pos[0] + 1, pos[1]) in player_two and (pos[0] + 2, pos[1]) in player_two and (
            pos[0] + 3, pos[1]) in player_two:
                return 2
            if (pos[0], pos[1] + 1) in player_two and (pos[0], pos[1] + 2) in player_two and (
            pos[0], pos[1] + 3) in player_two:
                return 2
            if (pos[0] + 1, pos[1] + 1) in player_two and (pos[0] + 2, pos[1] + 2) in player_two and (
            pos[0] + 3, pos[1] + 3) in player_two:
                return 2
            if (pos[0] - 1, pos[1] + 1) in player_two and (pos[0] - 2, pos[1] + 2) in player_two and (
            pos[0] - 3, pos[1] + 3) in player_two:
                return 2
