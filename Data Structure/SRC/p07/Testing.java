...
public class Testing {
	...
	public void chip9Manual() {
		...
		// test wire
		assert chip9.isAvailable(new Coord(0, 0), 1);
		assert chip9.isAvailable(new Coord(1, 0), 1);
		assert chip9.isAvailable(new Coord(0, 1), 2);
		assert chip9.isAvailable(new Coord(0, 0), 4);
		assert !chip9.isAvailable(new Coord(1, 0), 3);
		assert !chip9.isAvailable(new Coord(0, 1), 1);

		// test obstacle
		assert !chip9.isObstacle(new Coord(1, 1));
		assert !chip9.isObstacle(new Coord(2, 1));

		// test size
		assertEquals(dim, chip9.size());

		// test neighbor
		assertEquals("[(1, 0), (1, 2), (2, 1), (0, 1)]", chip9.neighbors(new Coord(1, 1)).toString());
	}
	...
	public void testBuildPath() {
		...
		obstacles.add(new Obstacle(4, 1, 8, 4));
		obstacles.add(new Obstacle(11, 2, 16, 3));
		obstacles.add(new Obstacle(14, 3, 14, 8));
		wires.add(new Wire(1, 4, 0, 8, 5));
		wires.add(new Wire(2, 8, 7, 16, 7));
		wires.add(new Wire(3, 12, 4, 15, 5));
		Chip chip3 = new Chip(dim, obstacles, wires);

		//System.out.println(chip3);

		List<Coord> coords_1 = PathFinder.buildPath(chip3.grid, dim, new Wire(1, 4, 0, 8, 5));
		assertEquals(12, coords_1.size());
		List<Coord> coords_2 = PathFinder.buildPath(chip3.grid, dim, new Wire(2, 8, 7, 16, 7));
		assertEquals(13, coords_2.size());
		List<Coord> coords_3 = PathFinder.buildPath(chip3.grid, dim, new Wire(3, 12, 4, 15, 5));
		assertEquals(13, coords_3.size());
		
		PathFinder.connectAllWires(chip3);
	}
	...
	public void chip3Layout() {
		...
		System.out.println("========== inputs/small_03.in ==========");
		System.out.println("before:\n" + chip3);
		if (layout == null || layout.size() != chip3.wires.size())
			System.out.println();
		System.out.println("after:\n" + chip3);
		chip3.layout();
		if (!validateLayout(layout, chip3))
			System.out.println("inputs/small_03.in");
	...
	@Test
	public void runSmallBenchmarks() {
		runBenchmarksFor("small");
	}
	
	@Test
	public void runMediumBenchmarks() {
		runBenchmarksFor("medium");
	}
	
	//@Test
	public void runBigBenchmarks() {
		runBenchmarksFor("big");
	}
	
	//@Test
	public void runHugeBenchmarks() {
