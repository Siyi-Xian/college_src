 ...
 * 
 * TODO: Complete the following three tasks in the order shown: 1. Implement the
 * Node.splitChild() method. 2. Handle the root full case in BTree.insert(). 3.
 * Ignore attempt to insert a duplicate key.
 */
...
class BTree {
	...
	class Node {
		...
		void splitChild(int i) {
			...
			int p = (size % 2 == 1) ? size / 2 : size / 2  - 1;

			int key = child.keys.get(p);
			this.keys.add(this.binarySearch(key), key);
			
			Node newChild_1 = new Node();
			newChild_1.keys.addAll(child.keys.subList(0, p));
			Node newChild_2 = new Node();
			newChild_2.keys.addAll(child.keys.subList(p + 1, child.keys.size()));
			
			for (Node n : child.children)
				if (n.keys.get(0) > key)
					newChild_2.children.add(n);
				else
					newChild_1.children.add(n);
			
			this.children.remove(i);
			this.children.add(i, newChild_1);
			this.children.add(i + 1, newChild_2);
	...
	public static void main(String... str) {
		...
		int[] nums = {12, 11, 1, 5, 3, 18, 13, 6, 20, 17, 7, 8, 9, 10, 2, 14, 15, 16, 19, 4};
		...
		for (int x : xs) {
			BTree.Location loc = bt.search(x);
			assert loc != null;
			assert loc.get() == x;
		}
