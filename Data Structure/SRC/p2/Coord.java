...
public class Coord implements Comparable<Coord> {
	...
	public List<Coord> neighbors(int size) {
		...
		Coord one = new Coord(this.x + 1, this.y);
		if (one.onBoard(size))
			coords.add(one);

		Coord two = new Coord(this.x - 1, this.y);
		if (two.onBoard(size))
			coords.add(two);

		Coord three = new Coord(this.x, this.y + 1);
		if (three.onBoard(size))
			coords.add(three);

		Coord four = new Coord(this.x, this.y - 1);
		if (four.onBoard(size))
			coords.add(four);
