...
task main() {
	...
	while(true) {
		...
		writeDebugStreamLine("Encoder Value: %d", SensorValue[encoder]);
		...
		while(!SensorValue[bump1]) {
			...
			while(SensorValue[encoder] > 0) {
				motor[leftMotor] = -50; //fullspeed backwards
				motor[rightMotor] = -50; //fullspeed backwards
			}
